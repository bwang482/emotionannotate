...
def predict(ci,gamma,kernel,wi,trfile,tfile,pfile,dir, emo):
    traincmd=["../libSVM/svm-train", "-c", "0.001", "-t", "2", "-w1", "1", "-w0", "1", "-q", trfile, dir+"/70-30/"+'/train.'+emo+'.model']
    traincmd[2]=ci
    traincmd[4]=kernel
    traincmd[8]=wi
#    traincmd[10]=gamma
...
#    model=trfile.split('/')[-1]+'.model'
    model=dir+"/70-30/"+'/train.'+emo+'.model'
    predcmd=["../libSVM/svm-predict", tfile, model, pfile]
    p = subprocess.Popen(predcmd, stdout=subprocess.PIPE)
...
#        trainfile = dir+"/70-30/"+emo+".train"
        testfile = dir+"/70-30/"+emo+".test"
        predfile = dir+"/70-30/"+emo+".test.pred"
#        truefile = dir+'/y_test'
        truefile = dir+"/70-30/"+emo+".test.true"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--d", dest="d", help="Output folder name", default='temp') # for testing
