    ...
    def transform(self, documents):
        ...
        for each_line in file01:
            if float(each_line.strip().split('\t')[1]) > 0:
                afinn_pos[each_line.strip().split('\t')[0]] = float(each_line.strip().split('\t')[1])
            elif float(each_line.strip().split('\t')[1]) < 0:
                afinn_neg[each_line.strip().split('\t')[0]] = float(each_line.strip().split('\t')[1])     
        file01.close()
        ...
        for each_line in file02:
            if float(each_line.strip().split('\t')[1]) > 0:
                gi_pos[each_line.strip().split('\t')[0]] = float(each_line.strip().split('\t')[1])
            elif float(each_line.strip().split('\t')[1]) < 0:
                gi_neg[each_line.strip().split('\t')[0]] = float(each_line.strip().split('\t')[1])
        file02.close()
        ...
        for i in [ line.strip().split('\t') for line in open("../lexicons/NRC-Hashtag-Emotion-Lexicon-v0.2.txt")]:
            if i[0] == 'anticipation':
               anticipation[i[1]] = float(i[2])
            if i[0] == 'anger':
               anger[i[1]] = float(i[2]) 
            if i[0] == 'disgust':
               disgust[i[1]] = float(i[2]) 
            if i[0] == 'fear':
               fear[i[1]] = float(i[2]) 
            if i[0] == 'joy':
               joy[i[1]] = float(i[2]) 
            if i[0] == 'sadness':
               sadness[i[1]] = float(i[2]) 
            if i[0] == 'surprise':
               surprise[i[1]] = float(i[2]) 
            if i[0] == 'trust':
               trust[i[1]] = float(i[2])
        ...
        for each_line in file1:
            if float(each_line.strip().split()[1]) > 0:
                lexicon1_pos[each_line.strip().split()[0]] = float(each_line.strip().split()[1])
            elif float(each_line.strip().split()[1]) < 0:
                lexicon1_neg[each_line.strip().split()[0]] = float(each_line.strip().split()[1])
        file1.close()
        ...
        for each_line in file2_pos:
            lexicon2_pos.append(each_line.strip())
        file2_neg = open('../lexicons/negative-words.txt', 'r')
        for each_line in file2_neg:
            lexicon2_neg.append(each_line.strip())
        file2_pos.close()
        file2_neg.close()
        ...
        for each_line in file4:
            items = each_line.strip().split()
            if float(items[1]) > 0:
                unihashtag_pos[items[0]] = tuple(np.array(items)[[1, 2]])
            elif float(items[1]) < 0:
                unihashtag_neg[items[0]] = tuple(np.array(items)[[1, 3]])
        file4.close()
        ...
        for each_line in file5:
            items = each_line.strip().split('\t')
            if float(items[1]) > 0:
                bihashtag_pos[tuple(items[0].split())] = tuple(np.array(items)[[1, 2]])
            elif float(items[1]) < 0:
                bihashtag_neg[tuple(items[0].split())] = tuple(np.array(items)[[1, 3]])
        file5.close()
        ...
        for each_line in file6:
            items = each_line.strip().split()
            if float(items[1]) > 0:
                unisenti140_pos[items[0]] = tuple(np.array(items)[[1, 2]])
            elif float(items[1]) < 0:
                unisenti140_neg[items[0]] = tuple(np.array(items)[[1, 3]])
        file6.close()
        ...
        for each_line in file7:
            items = each_line.strip().split('\t')
            if float(items[1]) > 0:
                bisenti140_pos[tuple(items[0].split())] = tuple(np.array(items)[[1, 2]])
            elif float(items[1]) < 0:
                bisenti140_neg[tuple(items[0].split())] = tuple(np.array(items)[[1, 3]])
        file7.close()
        ...
        for each_line in file8:
            items = each_line.strip().split('\t')
            if float(items[1]) > 0:
                pairhashtag_pos[frozenset(items[0].split('---'))] = tuple(np.array(items)[[1, 2]])
            elif float(items[1]) < 0:
                pairhashtag_neg[frozenset(items[0].split('---'))] = tuple(np.array(items)[[1, 3]])
        file8.close()
        ...
        for each_line in file9:
            items = each_line.strip().split('\t')
            if float(items[1]) > 0:
                pairsenti140_pos[frozenset(items[0].split('---'))] = tuple(np.array(items)[[1, 2]])
            elif float(items[1]) < 0:
                pairsenti140_neg[frozenset(items[0].split('---'))] = tuple(np.array(items)[[1, 3]])
        file9.close()
...
#        afinn_neg_lexicon_occ = [] #newly added
        ...
        for tweet in documents:
            ...
            afinn_pos_num = sum(map(lambda word: afinn_pos.get(word, 0), words))
            afinn_neg_num = sum(map(lambda word: afinn_neg.get(word, 0), words))
...
#            afinn_neg_num_occ = np.count_nonzero(np.asarray(map(lambda word: afinn_neg.get(word, 0), words)))
            gi_pos_num = sum(map(lambda word: gi_pos.get(word, 0), words))
            gi_neg_num = sum(map(lambda word: gi_neg.get(word, 0), words))
            anticipation_num = sum(map(lambda word: anticipation.get(word, 0), words))
            anger_num = sum(map(lambda word: anger.get(word, 0), words))
            disgust_num = sum(map(lambda word: disgust.get(word, 0), words))
            fear_num = sum(map(lambda word: fear.get(word, 0), words))
            joy_num = sum(map(lambda word: joy.get(word, 0), words))
            sadness_num = sum(map(lambda word: sadness.get(word, 0), words))
            surprise_num = sum(map(lambda word: surprise.get(word, 0), words))
            trust_num = sum(map(lambda word: trust.get(word, 0), words))
...
#            negunihashtag_temp = [] #newly added
            for each_word in words:
                if each_word in lexicon1_pos:
                    poslexicon1_num += lexicon1_pos[each_word]
#                    poslexicon1_temp.append(float(lexicon1_pos[each_word]))
#                    poslexicon1_num_occ += 1
                elif each_word in lexicon1_neg:
                    neglexicon1_num += lexicon1_neg[each_word]
#                    neglexicon1_temp.append(float(lexicon1_neg[each_word]))
#                    neglexicon1_num_occ += 1
                if each_word in lexicon2_pos:
                    poslexicon2_num += 1
                elif each_word in lexicon2_neg:
                    neglexicon2_num += 1
                if each_word in lexicon3_pos:
                    poslexicon3_num += lexicon3_pos[each_word]
#                    poslexicon3_temp.append(float(lexicon3_pos[each_word]))
#                    poslexicon3_num_occ += 1
                if each_word in lexicon3_neg:
                    neglexicon3_num += lexicon3_neg[each_word]
#                    neglexicon3_temp.append(float(lexicon3_neg[each_word]))
#                    neglexicon3_num_occ += 1
                if each_word in unihashtag_pos:
                    posunihashtag_num += float(unihashtag_pos[each_word][0])
#                    posunihashtag_num_occ += float(unihashtag_pos[each_word][1])
#                    posunihashtag_temp.append(float(unihashtag_pos[each_word][0]))
                elif each_word in unihashtag_neg:
                    negunihashtag_num += float(unihashtag_neg[each_word][0])
#                    negunihashtag_num_occ += float(unihashtag_neg[each_word][1])
#                    negunihashtag_temp.append(float(unihashtag_neg[each_word][0]))
                if each_word in unisenti140_pos:
                    posunisenti140_num += float(unisenti140_pos[each_word][0])
#                    posunisenti140_num_occ += float(unisenti140_pos[each_word][1])
#                    posunisenti140_temp.append(float(unisenti140_pos[each_word][0]))
                elif each_word in unisenti140_neg:
                    negunisenti140_num += float(unisenti140_neg[each_word][0]) 
...
#            negpairsenti140_temp = [] #newly added
            for each_item1 in words:
                for each_item2 in words:
                    if frozenset([each_item1, each_item2]) in pairhashtag_pos:
                        pospairhashtag_num += float(pairhashtag_pos[frozenset([each_item1, each_item2])][0])
...
#                        pospairhashtag_temp.append(float(pairhashtag_pos[frozenset([each_item1, each_item2])][0]))
                    ...
                    elif frozenset([each_item1, each_item2]) in pairhashtag_neg:
                        negpairhashtag_num += float(pairhashtag_neg[frozenset([each_item1, each_item2])][0])
...
#                        negpairhashtag_temp.append(float(pairhashtag_neg[frozenset([each_item1, each_item2])][0]))
                    ...
                    if frozenset([each_item1, each_item2]) in pairsenti140_pos:
                        pospairsenti140_num += float(pairsenti140_pos[frozenset([each_item1, each_item2])][0])
...
#                        pospairsenti140_temp.append(float(pairsenti140_pos[frozenset([each_item1, each_item2])][0]))
                    ...
                    elif frozenset([each_item1, each_item2]) in pairsenti140_neg:
                        negpairsenti140_num += float(pairsenti140_neg[frozenset([each_item1, each_item2])][0])
...
#                        negpairsenti140_temp.append(float(pairsenti140_neg[frozenset([each_item1, each_item2])][0]))
            ...
            for each_item1 in bigrams:
                for each_item2 in bigrams:
                    if frozenset([' '.join(each_item1), ' '.join(each_item2)]) in pairhashtag_pos:
                        pospairhashtag_num += float(pairhashtag_pos[frozenset([' '.join(each_item1), ' '.join(each_item2)])][0])
...
#                        pospairhashtag_temp.append(float(pairhashtag_pos[frozenset([' '.join(each_item1), ' '.join(each_item2)])][0]))
                    ...
                    elif frozenset([' '.join(each_item1), ' '.join(each_item2)]) in pairhashtag_neg:
                        negpairhashtag_num += float(pairhashtag_neg[frozenset([' '.join(each_item1), ' '.join(each_item2)])][0])
...
#                        negpairhashtag_temp.append(float(pairhashtag_neg[frozenset([' '.join(each_item1), ' '.join(each_item2)])][0]))
                    ...
                    if frozenset([' '.join(each_item1), ' '.join(each_item2)]) in pairsenti140_pos:
                        pospairsenti140_num += float(pairsenti140_pos[frozenset([' '.join(each_item1), ' '.join(each_item2)])][0])
...
#                        pospairsenti140_temp.append(float(pairsenti140_pos[frozenset([' '.join(each_item1), ' '.join(each_item2)])][0]))
                    ...
                    elif frozenset([' '.join(each_item1), ' '.join(each_item2)]) in pairsenti140_neg:
                        negpairsenti140_num += float(pairsenti140_neg[frozenset([' '.join(each_item1), ' '.join(each_item2)])][0])
...
#                        negpairsenti140_temp.append(float(pairsenti140_neg[frozenset([' '.join(each_item1), ' '.join(each_item2)])][0]))
            ...
            for each_item1 in words:
                for each_item2 in bigrams:
                    if frozenset([each_item1, ' '.join(each_item2)]) in pairhashtag_pos:
                        pospairhashtag_num += float(pairhashtag_pos[frozenset([each_item1, ' '.join(each_item2)])][0])
...
#                        pospairhashtag_temp.append(float(pairhashtag_pos[frozenset([each_item1, ' '.join(each_item2)])][0]))
                    ...
                    elif frozenset([each_item1, ' '.join(each_item2)]) in pairhashtag_neg:
                        negpairhashtag_num += float(pairhashtag_neg[frozenset([each_item1, ' '.join(each_item2)])][0])
...
#                        negpairhashtag_temp.append(float(pairhashtag_neg[frozenset([each_item1, ' '.join(each_item2)])][0]))
                    ...
                    if frozenset([each_item1, ' '.join(each_item2)]) in pairsenti140_pos:
                        pospairsenti140_num += float(pairsenti140_pos[frozenset([each_item1, ' '.join(each_item2)])][0])
...
#                        pospairsenti140_temp.append(float(pairsenti140_pos[frozenset([each_item1, ' '.join(each_item2)])][0]))
                    ...
                    elif frozenset([each_item1, ' '.join(each_item2)]) in pairsenti140_neg:
                        negpairsenti140_num += float(pairsenti140_neg[frozenset([each_item1, ' '.join(each_item2)])][0])
...
#            pospairsenti140_max.append(max(pospairsenti140_temp))
